...
export const EN: Record<string, string> = {
  ...
  'Аудит цен — сервис с прогнозируемой годовой экономией 465 млн. рублей, который помогает проверять закупочные цены на соответствие рынку':
    'Price Audit is a service with a projected annual saving of 465M rubles that helps verify purchase prices against the market',
  'Он разрабатывался почти год, и за это время требования к интерфейсу и сценарии использования несколько раз менялись. Команда постоянно была на связи с заказчиком, но обратная связь от конечных пользователей оставалась негативной. Если бы проблемы не разобрали до релиза новой версии, сервис рисковал не решить задачи пользователей и столкнуться с отказом от эксплуатации на стороне заказчика. Нужно было понять, закрывает ли новая версия сервиса реальные потребности пользователей в сценарии аудита спецификации, и что мешает работать с интерфейсом самостоятельно':
    "It had been in development for almost a year, and the interface requirements and usage scenarios changed several times along the way. The team stayed in constant touch with the client, but feedback from end users remained negative. If the problems hadn't been sorted out before the new version shipped, the service risked failing to solve users' problems and facing rejection on the client's side. We needed to find out whether the new version of the service actually met users' needs in the specification audit scenario, and what stopped them from using the interface on their own",
